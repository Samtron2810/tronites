...
const ChatModal = ({
  isOpen, onClose, selectedChat, messages, threadLoading,
  user, onlineUsers, handleSendMessage, handleImageSelect,
  handleDeleteMessage, messageDeletingId, messageText,
  setMessageText, imagePreview, setImagePreview, isSending,
  fileInputRef, scrollRef,
}) => {
  if (!isOpen || !selectedChat) return null;
  const activeUser = selectedChat.otherUser;
  const activeIsOnline = activeUser ? onlineUsers.includes(activeUser._id) : false;

  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center p-4 pt-12">
      <div className="absolute inset-0 bg-black/40 backdrop-blur-sm" onClick={onClose} />

      <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-[88vh] flex flex-col overflow-hidden border border-stroke">
        {/* Header */}
        <div className="flex items-center gap-3 px-4 py-3 border-b border-stroke">
          ...
          <div className="relative shrink-0">
            ...
            />
            <span className={`absolute bottom-0 right-0 h-2.5 w-2.5 rounded-full border-2 border-white ${activeIsOnline ? "bg-primary-400" : "bg-gray-300"}`} />
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold text-ink truncate">{activeUser?.name}</p>
            <p className={`text-xs ${activeIsOnline ? "text-primary-600" : "text-ink-muted"}`}>
            ...
            <div className="space-y-3 animate-pulse">
              {[1, 2, 3].map((i) => (
                <div key={i} className={`flex ${i % 2 === 0 ? "justify-end" : "justify-start"}`}>
                  <div className={`h-9 rounded-2xl ${i % 2 === 0 ? "bg-primary-200 w-40" : "bg-gray-200 w-52"}`} />
                </div>
              ))}
            </div>
          )}
            ...
            return (
              <div key={message._id} className={`flex ${isMine ? "justify-end" : "justify-start"}`}>
                <div className="flex flex-col max-w-[75%] gap-1">
                  {message.image && (
                    <img src={message.image} alt="message" className={`rounded-xl max-w-xs h-auto object-cover ${isMine ? "ml-auto" : ""}`} />
                  )}
                  {message.text && (
                    <div className={`px-4 py-2.5 rounded-2xl text-sm whitespace-pre-wrap ${
                      isMine ? "bg-primary-600 text-white self-end rounded-br-sm" : "bg-white text-ink self-start rounded-bl-sm border border-stroke"
                    }`}>
                      {message.text}
                    </div>
                  )}
                  <div className={`flex items-center gap-1.5 ${isMine ? "justify-end" : "justify-start"}`}>
                    ...
                    {isMine && (
                      <>
                        ...
                        <span className={message.read ? "text-primary-400" : "text-ink-muted"}>
                          {message.read ? <FaCheckDouble size={11} /> : <FiCheck size={11} />}
                        </span>
                      </>
                    )}
                  </div>
