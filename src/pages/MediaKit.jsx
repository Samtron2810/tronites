...
const Metric = ({ value, label, icon: Icon, className = "" }) => (
  <div className={`flex flex-col items-center text-center ${className}`}>
    <div className="flex items-center gap-1 text-ink">
      ...
      <span className="text-2xl font-extrabold tracking-tight leading-none">{value}</span>
    </div>
...
const MediaKit = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const { data } = await api.get(`/creator-monetization/media-kit/${creatorId}`);
  ...
  const { creator, reach, engagement, subscriptionPlan, topPosts, generatedAt } = kit;
    ...
    <div className="max-w-2xl mx-auto px-4 py-6 space-y-5">
      ...
      <div className="bg-card border border-stroke rounded-3xl overflow-hidden">
        ...
        <div className="px-6 pt-6 pb-5">
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-1.5 flex-wrap">
                <h1 className="text-xl font-extrabold text-ink leading-none">{creator.name}</h1>
                {creator.isVerified && (
                  <VerifiedBadge verifications={[{ type: "creator" }]} size={16} />
                )}
              </div>
              <p className="text-sm text-ink-muted mt-0.5">@{creator.username}</p>
              {creator.location && (
                <p className="text-xs text-ink-muted mt-1 flex items-center gap-1">
                  <FaMapMarkerPin size={10} /> {creator.location}
                </p>
              )}
            </div>
          ...
          {creator.bio && (
            <p className="text-sm text-ink-muted mt-4 leading-relaxed">{creator.bio}</p>
          )}
      ...
      <div className="bg-card border border-stroke rounded-2xl px-6 py-5">
        <h2 className="text-xs font-semibold text-ink-muted uppercase tracking-widest mb-4">Reach</h2>
        <div className="grid grid-cols-3 gap-4 divide-x divide-stroke">
          <Metric value={fmtNum(reach.followers)} label="Followers" icon={FaUsers} />
          <Metric value={fmtNum(reach.totalPosts)} label="Posts" icon={FaChartBar} className="pl-4" />
          <Metric value={fmtNum(reach.activeSubscribers)} label="Subscribers" icon={FaStar} className="pl-4" />
        </div>
        <div className="grid grid-cols-3 gap-4 divide-x divide-stroke mt-4 pt-4 border-t border-stroke">
          <Metric value={fmtNum(reach.totalLikes)} label="Total likes" icon={FaHeart} />
          <Metric value={fmtNum(reach.totalReposts)} label="Reposts" className="pl-4" />
          <Metric value={fmtNum(reach.totalComments)} label="Comments" className="pl-4" />
        </div>
      </div>

      {/* Engagement */}
      <div className="bg-card border border-stroke rounded-2xl px-6 py-5">
        <h2 className="text-xs font-semibold text-ink-muted uppercase tracking-widest mb-4">Engagement</h2>
        <div className="grid grid-cols-3 gap-4 divide-x divide-stroke">
          <div className="text-center">
            <p className={`text-2xl font-extrabold leading-none ${engagementColor}`}>
      ...
      {subscriptionPlan && (
        <div className="bg-card border border-stroke rounded-2xl px-6 py-5">
          <h2 className="text-xs font-semibold text-ink-muted uppercase tracking-widest mb-3">Subscription</h2>
          <div className="flex items-center justify-between">
            <div>
              <p className="font-semibold text-ink text-sm">{subscriptionPlan.name}</p>
              {subscriptionPlan.perks && (
                <p className="text-xs text-ink-muted mt-1">{subscriptionPlan.perks}</p>
              )}
            </div>
            <p className="text-xl font-extrabold text-ink">
              ₦{subscriptionPlan.priceNgn?.toLocaleString()}<span className="text-xs text-ink-muted font-normal">/mo</span>
            </p>
          </div>
        </div>
      )}
      ...
      {topPosts?.length > 0 && (
        <div className="bg-card border border-stroke rounded-2xl px-6 py-5 space-y-3">
          <h2 className="text-xs font-semibold text-ink-muted uppercase tracking-widest">Top posts (last 30 days)</h2>
          {topPosts.map((p, i) => (
            <div key={p._id} className="flex items-start gap-3 py-3 border-b border-stroke last:border-0">
              <span className="text-xs font-bold text-ink-muted w-4 shrink-0 pt-0.5">#{i + 1}</span>
              <div className="flex-1 min-w-0">
                <p className="text-sm text-ink line-clamp-2">{p.text || "(Image post)"}</p>
                <div className="flex items-center gap-4 mt-1.5">
                  <span className="text-xs text-ink-muted flex items-center gap-1">
                    <FaHeart size={10} /> {fmtNum(p.likesCount)}
                  </span>
                  <span className="text-xs text-ink-muted">{fmtNum(p.commentsCount)} comments</span>
                </div>
              </div>
            </div>
          ))}
      ...
      {creator.openToCollabs && (
        <div className="bg-primary-50 dark:bg-primary-950/20 border border-primary-200 dark:border-primary-800 rounded-2xl px-6 py-5 text-center space-y-3">
          ...
          <p className="text-sm font-semibold text-ink">Interested in a collaboration?</p>
